...
parser.add_argument('--src',  default='ucf101', 
                    help='source domain')
parser.add_argument('--tar',  default='hmdb51', 
                    help='target domain')
    ...
    for i in range(epoch_size):
        ...
        f_mean, f_logvar, f_post, \
            z_post_mean, z_post_logvar, z_post, \
                z_prior_mean, z_prior_logvar, z_prior, \
                    recon_x, pred_domain_all, pred_video_class, _ = model(x, beta)


        # (I) calculate sequential VAE loss
        ...
        if opt.weight_MI:
            _logq_f_tmp = utils.log_density(f_post.unsqueeze(0).repeat(opt.num_segments, 1, 1).view(opt.num_segments, 2*opt.batch_size, 1, opt.f_dim),
                                            f_mean.unsqueeze(0).repeat(opt.num_segments, 1, 1).view(opt.num_segments, 1, 2*opt.batch_size, opt.f_dim),
            ...
            _logq_z_tmp = utils.log_density(z_post.transpose(0, 1).view(opt.num_segments, 2*opt.batch_size, 1, opt.z_dim),
                                            z_post_mean.transpose(0, 1).view(opt.num_segments, 1, 2*opt.batch_size, opt.z_dim),
...
def validate(val_loader, model):
    ...
    for i in range(val_size):
        ...
        with torch.no_grad():
            _, _, _, _, _, _, _, _, _, _, _, pred_video_class, _ = model(x, [0]*len(opt.beta))
...
def main(opt):
    ...
    model_config = 'TranSVAE-%s-%s-%s(%s)-%s-addfc_%d-dropout_%.2f-useAttn_%s-usePsue_%s(%.4f)-addlossDA_%s-frames_%d-prior_%s' % (opt.src, opt.tar, opt.input_type, opt.backbone,
                                                                                                                                   opt.frame_aggregation, opt.add_fc, opt.dropout_rate, opt.use_attn, opt.use_psuedo, opt.tar_psuedo_thre, opt.add_loss_DA, opt.num_segments, opt.prior_sample)
    ...
    val_list = '%s/%s/list/%s/list_val_%s-%s_%s.txt' % (
        opt.data_root, opt.tar, opt.backbone, opt.src, opt.tar, opt.backbone)
