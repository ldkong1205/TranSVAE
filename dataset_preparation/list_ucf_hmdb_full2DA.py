...
parser.add_argument('modality', type=str, 
					help='rgb | flow')
parser.add_argument('--sp', default=1, help='split #',
...
parser.add_argument('--class_file', type=str, default='class.txt',
                    help='process the classes only in the class_file', required=True)
...
def gen_list_DA(path_input_list, class_indices_DA, class_names_DA, list_type):
    path_output = args.data_path + args.dataset + '/' + 'list_' + \
        args.dataset + '_' + list_type + args.suffix + '.txt'
    ...
    for line in open(path_input_list):
        ...
        if args.dataset == 'hmdb51':
            name_video_short = name_video.rsplit(
                '_', 6)[0]  # remove the suffix
            name_str = name_video_short.rsplit(
                '_', 2)[-2:]  # remove the prefix
            class_str = '_'.join(name_str)  # join the strings

            if class_str.split('_')[1] in class_names_DA:
                check_class = True
                id_video_DA = class_indices_DA[class_names_DA.index(
                    class_str.split('_')[1])]
        ...
        if check_class:
            ...
            if args.method_read == 'frame':
                len_video = str(
                    len(os.listdir(path_dataset + '/' + args.frame_in + '/' + name_video)))

            line_new = path_dataset + '/' + args.frame_in + '/' + \
                name_video + ' ' + len_video + ' ' + str(id_video_DA) + '\n'
...
path_split_folder = args.data_path + \
    args.dataset + '/' + args.dataset + '_splits/'
path_train_list = path_split_folder + args.dataset + '_' + \
    args.modality + '_train_split_' + str(args.sp) + '.txt'
path_val_list = path_split_folder + args.dataset + '_' + \
    args.modality + '_val_split_' + str(args.sp) + '.txt'

class_indices = [int(line.strip().split(' ', 1)[0])
